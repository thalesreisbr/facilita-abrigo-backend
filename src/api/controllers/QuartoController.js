...
exports.filtrar = async (request, response, next) => {
	let { data_inicial, data_final, cidade, caracteristicas, abrigo_id } = request.query;
	if(caracteristicas)
		caracteristicas = caracteristicas.split(',');
	try {
		const instancias = await QuartoServices.filtrar(data_inicial, data_final, cidade, caracteristicas, abrigo_id);
